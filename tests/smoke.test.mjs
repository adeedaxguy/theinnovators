...
test("USA uses its dedicated Canva-aligned layout and map overlay", async () => {
  ...
  assert.match(page, /usa-full-video-library/);
  assert.match(page, /Programs, Grants and Funding/);
  assert.match(page, /Institutions, Policy and State Ecosystems/);
  assert.match(page, /Industry/);
  assert.match(page, /Stage/);
  assert.match(page, /Funding/);
  assert.match(page, /usa-agent-insights/);
  assert.match(page, /content\.resourceSections \?\? \[\]/);
...
test("world follows the Canva three-column dashboard and separate map layer", async () => {
  ...
  assert.match(page, /world-region-tabs/);
  assert.match(page, /North America/);
  assert.match(page, /world-country-stage/);
  assert.match(page, /world-summits-section/);
  assert.match(page, /Global Innovation Stories/);
  assert.match(page, /Policy, Industry and Ecosystem Briefings/);
  assert.match(page, /ScrollRail className="world-news-rail"/);
  assert.match(page, /ScrollRail className="world-organization-grid"/);
  assert.match(page, /ScrollRail className="world-leaders-grid"/);
